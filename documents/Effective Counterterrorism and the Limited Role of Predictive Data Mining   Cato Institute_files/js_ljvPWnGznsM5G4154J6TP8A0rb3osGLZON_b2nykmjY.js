...
(function ($) {
  Drupal.behaviors.upstatement = {
    attach: function (context, settings) {
      $('.share-tools-list li a, .pub-page-share-list li a').click(function () {
        var targetUrl;
        var network;
        var action;
        var url;
        ...
        if ($(this).parent().hasClass('reddit')) {
          network = 'Reddit';
          action = 'submit';
        }
        ...
        if (targetUrl) {
          url = $.getUrlVar(href, targetUrl);
        }
        else {
          url = decodeURIComponent(href.replace('http://reddit.com/', ''));
        }
        ga('send', 'social', network, action, url);
      });
      var catoMegaMenu = $('#block-catomegamenu-catomegamenu');
      catoMegaMenu.find('.block-panels-mini a').click(function () {
        var currentMegaMenu = $(this).parents('.block-panels-mini').attr('id').match(/block-panels-mini-(.*)-megamenu-layout/);
        ga('send', 'event', 'Megamenu Clicks', currentMegaMenu[1],
          $(this).attr('href'));
      });
      catoMegaMenu.find('div.content > div.item-list > ul.menu > li.expanded > a').click(function () {
        ga('send', 'event', 'Megamenu Top Clicks', $(this).text());
      });
      $('#block-menu-menu-submenu').find('.leaf > a').click(function () {
        ga('send', 'event', 'Submenu Clicks', $(this).text());
      });
      ...
      var ww = $(window).width();
      var megaMenu = function (width) {
        if ((width >= 860) && $.support.leadingWhitespace) {
          var hoverConfig = {
            over: function () {
              $(this).children('.item-list').css('display', 'block');
              ga('send', 'event', 'Megamenu Hovers',
                $(this).find(' > a').text());
            },
            timeout: 50,
            interval: 200,
          ...
          var megaSection = catoMegaMenu.find('div.content > div.item-list > ul.menu > li.expanded');
          megaSection.hoverIntent(hoverConfig);
          megaSection.on('touchstart', function () {
            $(this).unbind();
          });
          $header = $('#header');
          if ($header.hasClass('search-active')) {
      ...
      if ($('.region-sidebar-first .sidenav').size()) {
        ...
        setRaHeight();
        $('.region-sidebar-first .sidenav').fixedScroll({
      ...
      //fixed-scroll
      $('.region-sidebar-first .fixedscroll').fixedScroll({
        container: '#page-area',
        forceTop: true,
        offset: 20,
        shrinkContainer: 100
      });
      $('.pub-page-share-list').fixedScroll({
        container: '.pub-page #page-area',
        forceTop: true,
        offset: 20,
        shrinkContainer: 100,
        breakPoint: 1030
